...
export const ACCELERATOR_HELP = `codex-accelerator — local Delta Mode MCP server
...
export function parseAcceleratorArgs(args = process.argv.slice(2), env = process.env) {
  ...
  for (; cursor < args.length; cursor++) {
    ...
    const key = { '-C': 'workspace', '--workspace': 'workspace', '--database': 'database', '--max-file-bytes': 'maxFileBytes', '--max-output-bytes': 'maxOutputBytes' }[arg];
    if (!key) throw new Error(`Unknown argument: ${arg}`);
    const value = args[++cursor];
    if (!value || value.startsWith('--')) throw new Error(`Missing value for ${arg}.`);
    options[key] = key.startsWith('max') ? positiveInteger(value, arg) : value;
  }
  if (options.command !== 'mcp') throw new Error('Only the mcp command is available in Delta Mode v0.1.');
...
export async function createAcceleratorRuntime(options) {
  ...
  let closed = false;
  return {
    workspaceRoot, repositoryId, store, sessionManager, rawOutputStore, fileDeltaService, commandDeltaService,
    createServer: () => createAcceleratorMcpServer({ fileDeltaService, commandDeltaService, sessionManager, rawOutputStore }),
    async close() {
      if (closed) return;
      closed = true;
      try {
